...
public class Lexer
{
    ...
    public static void main(String args[])
    {
        ...
        try
        {
            ...
            {
                tok = lex.nextToken();
                String p = "L: " + tok.getLeftPosition() + " R: "
                        + tok.getRightPosition() + "  "
                        + TokenType.tokens.get(tok.getKind()) + " ";
                if ((tok.getKind() == Tokens.Identifier)
                        || (tok.getKind() == Tokens.INTeger) || (tok.getKind() == Tokens.FLOATing))
                    p += tok.toString();
     ...
     * 
     * @param id
     *            is the String just scanned - it's either an id or reserved
     *            word
     * @param startPosition
     *            is the column in the source file where the token begins
     * @param endPosition
     *            is the column in the source file where the token ends
     ...
     * 
     * @param number
     *            is the int String just scanned
     * @param startPosition
     *            is the column in the source file where the int begins
     * @param endPosition
     *            is the column in the source file where the int ends
     * @return the int Token
     */
    public Token newNumberToken(String number, int startPosition,
            int endPosition)
    {
        return new Token(startPosition, endPosition,
    ...
    //Same as newNumberToken but for floats
    public Token newFloatToken(String number, int startPosition,
            int endPosition)
    {
        return new Token(startPosition, endPosition,
     ...
     * out comments which begin with two slashes and after modification 2 two
     * dashes
     * 
     * @param s
     *            is the String representing the token
     * @param startPosition
     *            is the column in the source file where the token begins
     * @param endPosition
     *            is the column in the source file where the token ends
    ...
    public Token makeToken(String s, int startPosition, int endPosition)
    {
        ...
        Symbol sym = Symbol.symbol(s, Tokens.BogusToken); // be sure it's a
                                                          // valid token
        if (sym == null)
        {
    ...
    { // ch is always the next char to process
        ...
        if (Character.isDigit(ch) || ch == '.')
        {
            ...
            try
            {
                ...
                } while (Character.isDigit(ch) || (dotCount == 1 && ch == '.'));
            } catch (IOException e)
            {
                atEOF = true;
            }
